(function($) {
	...
	window.optGenerator = {
		...
		getStorage: function(storageName) {
			...
			if( tryCookies ) {
				if( !$.onepress.tools.cookie(storageName) ) {
					return false;
				}
		...
		setStorage: function(storageName, value) {
			...
			if( tryCookies ) {
				$.onepress.tools.cookie(storageName, value, {
					expires: 86400,
					path: "/"
				});
			}
			...
			for( var i in options ) {
				if( i !== 'buttons' ) {
					...
					if( typeof options[i]['lazy'] !== 'undefined' ) {
						arr['slg-buttons-lazy'] = options[i]['lazy'];
					}
				}
			}
			return arr;
		...
		updateLockerOptions: function() {
			...
			if( lang !== 'en_US' ) {
				...
				if( $.inArray("twitter-tweet", self._buttonsOrder) || $.inArray("twitter-follow", self._buttonsOrder) ) {
					if( !self._options.twitter ) {
						self._options.twitter = {};
					}
		...
		converteFildIdToLockerOption: function(fildId) {
			...
			var itemValue = $("#" + fildId).attr('type') == 'checkbox'
				? $("#" + fildId).is(":checked")
				: $("#" + fildId).val();
