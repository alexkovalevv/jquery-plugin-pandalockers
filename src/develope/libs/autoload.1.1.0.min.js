	...
	window.__onpwgtInstaller = {
		...
		loadLibs: function() {
			...
			$("<link/>", {
				rel: "stylesheet",
				type: "text/css",
				href: self._LIBS_URL + self._CSS_DIR + 'pandalocker.' + build + '.2.1.3.min.css'
			}).appendTo("head");

			// Ядро
			promise = self._loadScript(promise, self._LIBS_URL + 'pandalocker.' + build + '.2.1.3.min.js', function() {
				if( !$.pandalocker.lse ) {
					$.pandalocker.lse = {};
				}
